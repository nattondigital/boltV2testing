/*
  # Add Employee Name and Global Date/Time Variables to Triggers

  1. Changes
    - Update expense triggers to include employee_name from admin_users lookup
    - Add global {{current_date}} and {{current_time}} variables to all trigger_data
    - Add {{submission_date}} as alias for created_at for better clarity

  2. Notes
    - employee_name will be looked up from admin_users table
    - current_date format: YYYY-MM-DD (e.g., 2025-11-12)
    - current_time format: HH:MM:SS (e.g., 14:30:45)
    - submission_date format: YYYY-MM-DD HH:MM:SS (full timestamp)
    - These global variables can be used in ALL WhatsApp templates going forward

  3. Global Variables Available
    - {{current_date}}: Current date when trigger fires
    - {{current_time}}: Current time when trigger fires
    - {{submission_date}}: Formatted created_at timestamp
    - {{employee_name}}: Name of the employee (for expenses)
*/

-- ==========================================
-- EXPENSE TRIGGERS (Add employee_name and global date/time)
-- ==========================================
...
BEGIN
  ...
  -- Build trigger data with all fields plus employee_name and global date/time
...
COMMENT ON FUNCTION trigger_workflows_on_expense_add() IS 'Triggers workflows for expense additions with employee_name lookup and global date/time variables';
COMMENT ON FUNCTION trigger_workflows_on_expense_update() IS 'Triggers workflows for expense updates with employee_name lookup and global date/time variables';
COMMENT ON FUNCTION trigger_workflows_on_expense_delete() IS 'Triggers workflows for expense deletions with employee_name lookup and global date/time variables';
